...
ALTER TABLE NATION_TBL 
    DROP PRIMARY KEY;   --테이블의 기본키는 오직 1개이므로 제약조건의 이름을 몰라도 삭제 가능 
ALTER TABLE NATION_TBL
...
ALTER TABLE SCHEDULE_TBL
    DROP CONSTRAINT FK_SCHDULE_EVENT;
...
ALTER TABLE SCHEDULE_TBL
    ADD CONSTRAINT FK_SCHDULE_EVENT FOREIGN KEY(E_CODE)
        REFERENCES EVENT_TBL(E_CODE)
            ON DELETE SET NULL;
-- 연습. NATION_TBL의 기본키 제거하기
-- 외래키(FK)에 의해서 참조중인 기본키(PK)는 "반드시" 외래키를 먼저 삭제해야 한다.
ALTER TABLE PLAYER_TBL
    DROP CONSTRAINT FK_PLAYER_NATION;
ALTER TABLE SCHEDULE_TBL
    DROP CONSTRAINT FK_SCHEDULE_NATION;
