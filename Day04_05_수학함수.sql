...
-- 8. 원하는 자릿수로 반올림
--      REOUND(A, [DIGIT]) : 실수 A를 DIGIT 자릿수로 반올림 값을 반환, DIGIT 생략하면 정수로 반올림
SELECT 
          ROUND(123.456)     -- 정수로 반올림
        , ROUND(123.456, 1)  -- 소수 1자리 남기고 반올림
        , ROUND(123.456, 2)  -- 소수 2자리 남기고 반올림
        , ROUND(123.456, -1) -- 1의 자리에서 반올림
        , ROUND(123.456, -2) -- 10의 자리에서 반올림
        FROM
            DUAL;
            
-- 9. 원하는 자릿수로 절사 (공부해 두기)
--      TRUNC(A, [DIGIT]) : 실수 A를 DIGIT 자릿수로 절사 값을 반환, DIGIT 생략하면 정수로 절사
SELECT 
          TRUNC(123.456)     -- 정수로 절사
        , TRUNC(123.456, 1)  -- 소수 1자리 남기고 절사
        , TRUNC(123.456, 2)  -- 소수 2자리 남기고 절사
        , TRUNC(123.456, -1) -- 1의 자리에서 절사
        , TRUNC(123.456, -2) -- 10의 자리에서 절사
        FROM
            DUAL;

-- 생각해보기
-- FLOOR 함수 TRUNC 함수는 무엇이 다른가
-- FLOOR : 작은 정수 찾기
-- TRUNC : 보이는 소수점 자르기
SELECT
        FLOOR(1.9)
