...
-- BUY_PROC 프로시저 정의
CREATE OR REPLACE PROCEDURE BUY_PROC(CNO IN CUST_TBL.C_NO%TYPE, PCODE IN PROD_TBL.P_CODE%TYPE, BUY_AMOUNT IN BUY_TBL.B_AMOUNT%TYPE)
IS
BEGIN
    -- 1) 구매내역 테이블에 구매 내역을 추가(INSERT)한다.
    ...
    UPDATE CUST_TBL SET C_POINT = C_POINT + CEIL((SELECT P_PRICE FROP FROM PROD_TBL WHERE P_CODE = PCODE) * BUY_AMOUNT * 0.1) WHERE C_NO = CNO;
