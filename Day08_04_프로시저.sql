...
-- 프로시저2 : 사원번호가 100인 사원의 FIRST_NAEM, LAST_NAME, SALARY를 조회
...
-- 프로시저3 : 사원번호를 전달하면 해당 사원의 FIRST_NAEM, LAST_NAME, SALARY를 조회    
-- 입력 파라미터
-- 1. 프로시저 값을 정달할 때 사용한다
-- 2. 형식 : 변수명 IN 타입
...
BEGIN
    SELECT FIRST_NAME, LAST_NAME
      INTO FNAME, LNAME -- 이 곳에 출력 파라미터에 값이 저장된다
      FROM EMPLOYEES
      WHERE EMPLOYEE_ID = 100;
END;

-- 호출
-- FNAME과 LNAME값을 저장할 변수를 선언한뒤, 프로시저에 전달해야 한다.
DECLARE
    FNAME EMPLOYEES.FIRST_NAME%TYPE;
    LNAME EMPLOYEES.LAST_NAME%TYPE;
BEGIN
    PROC4(FNAME, LNAME);    -- PLSQL 내부에서 프로시저를 호출할 때는 EXCUTE를 생략해야한다
    DBMS_OUTPUT.PUT_LINE(FNAME || ',' || LNAME);
END;

    
-- 프로시저5. 사원번호를 전달하면, 해당 사원의 FIRST_NAME이 출력 파라미터 FNAME에 저장되도록 작성.
-- 없는 사원번호가 전달되면 FNAME에 NONAME이 저장되도록 작성 
CREATE OR REPLACE PROCEDURE PROC5(EMP_ID IN EMPLOYEES.EMPLOYEE_ID%TYPE, FNAME OUT EMPLOYEES.FIRST_NAME%TYPE)
IS 
BEGIN
    SELECT FIRST_NAME
      INTO FNAME
...
DECLARE
    FNAME EMPLOYEES.FIRST_NAME%TYPE;
BEGIN
    PROC5(100, FNAME);    -- PLSQL 내부에서 프로시저를 호출할 때는 EXCUTE를 생략해야한다
    DBMS_OUTPUT.PUT_LINE(FNAME || '입니다.');
END;    
