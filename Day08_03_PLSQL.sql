...
/*
    테이블 복사하기
    1. CREATE TABLE과 복사할 데이터의 조회(SELECT)를 사용한다
...
/*
    ...
    2. 종류
        ...
        3) 레코드 변수 : 2개 이상의 칼럼을 합쳐서 하나의 타입으올 지정한다.
...
DECLARE
    -- 레코드 변수 정의하기
    TYPE MY_RECORD_TYPE IS RECORD(  -- 타입명 : MY_RECORDE_TYPE
        FNAME EMPLOYEES.FIRST_NAME%TYPE,
        LNAME EMPLOYEES.LAST_NAME%TYPE,
        SAL EMPLOYEES.SALARY%TYPE
    );
...
-- EMPLOYEE_ID가 150인 사원의 SALARY가 15000이상이면 '고액연봉', 아니면 '보통연봉'을 출력하시오
DECLARE
...
-- EMPLOYEE_ID가 150인 사원의 COMMISSION_PCT가 0이면 '커미션없음', 아니면 실제 커미션(COMMISIION_PCT * SALARY)을 출력하시오
DECLARE
    EMP_ID EMPLOYEES.EMPLOYEE_ID%TYPE;
...
BEGIN
    EMP_ID := 150;
    SELECT NVL(COMMISSION_PCT, 0), SALARY맛없어 보이던대
    INTO COMM_PCT, SAL
    FROM EMPLOYEES 
...
-- EMPLOYEE_ID가 150인 사원의 PHONE_NUMBER에 따은 지역을 출력하시오.
-- 011 : MOBILE
-- 515 : EAST
-- 590 : WEST
-- 603 : SOUTH
...
--
DECLARE
    EMP_ID EMPLOYEES.EMPLOYEE_ID%TYPE;  -- 합칠수 없어서
    
    TYPE FULLNAME_TYPE IS RECORD(
    FNAME EMPLOYEES.FIRST_NAME%TYPE,
    LNAME EMPLOYEES.LAST_NAME%TYPE
);
   FULLN FULLNAME_TYPE;
BEGIN
    ...
    WHILE EMP_ID <= 206 LOOP
    SELECT FIRST_NAME, LAST_NAME
      INTO FULLN    -- 여기로 저장해라 FN, LN을
      FROM EMPLOYEES
      WHERE EMPLOYEE_ID = EMP_ID;
    DBMS_OUTPUT.PUT_LINE(FULLN.FNAME || ' ' || FULLN.LNAME);
    EMP_ID := EMP_ID + 1;
    END LOOP;
END;
...
BEGIN
    ...
    IF MODULAR = 0 THEN
     ...
     ELSE
        ...
        END IF;
    END IF;
    DBMS_OUTPUT.PUT_LINE(N || ' 은' || MESSAGE || '입니다.');
   END LOOP;
END;
...
-- DEPARTMENT_ID 가 50인 사원들의 목록을 DEPT50 테이블로 복사하시오
-- 1) DEPT50 테이블 만들기
-- 2) 행 변수로 EMPLOYEES 테이블의 정보 읽기
-- 3) DEPARTMENT_ID가 50이면 행 변수에 저장되 내용을 DEPT50 테이블에 INSERT
DROP TABLE DEPT50;
CREATE TABLE DEPT50
...
-- 1부터 3의배수를 제외한 정수 값을 누적하시오. 누적 값이 100을 초과하면 그만 누적하고 어디까지 누적했는지 출력하시오.
DECLARE
    N NUMBER;
    TOTAL NUMBER;
    MODULAR NUMBER(1);
