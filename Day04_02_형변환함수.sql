/*
    DUAL 테이블
    1. DUMMY 칼럼에 1개 + 'X' 값을 1개 가지고 있는 테이블이다.
    2. 아무 일도 안하는 테이블이다
    3. 오라클은 SELECT - FROM절이 필수이므로 언제나 테이블이 필요하다.
        테이블이 필요 없는 값을 조회할 떄 DUAL 테이블을 사용한다.
*/
...
SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME
  FROM EMPLOYEES
  WHERE EMPLOYEE_ID = '150';    -- EMPLOYEE_ID = TO_NUMBER('150')으로 자동 변환 뒤 처리한다.
...
/* 3. 날짜로 변환하기(TO_DATE)*/
-- 날짜와 시간을 원하는 형식으로 표시하는 함수가 아님을 주의!
-- '05/06/07' 이 날짜는 알려주기 전에는 해석이 안되는 날짜이다
-- 어떤 날짜를 어떤 방법으로 해석해야 하는지 알려주는 함수가 TO_DATE이다
SELECT 
...
-- EMPLOYEES 테이블에서 00/01/01 ~ 05/12/31 사이에 입사한 사원 조회
...
SELECT 
    ...
    , TO_CHAR(DT1, 'YYYY-MM-DD')  -- 원하는 형식이 젹용됨
    FROM
        SAMPLE_TBL;
