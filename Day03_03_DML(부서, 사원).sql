/*
    ...
    4. DML 사용 후에는 COMMIT(저장) 또는 ROLLBACK 처리를 해야 한다.
    5. 종류
...
CREATE SEQUENCE DEPARTMENT_SEQ
    ...
    NOMAXVALUE      -- 번호의 상한선이 없다. (생략 가능) MAXVALUE 100 : 번호를 100까지만 생성
    NOMINVALUE      -- 번호의 하한선이 없다. (생략 가능) MINVALUE 100 : 번호의 최소값이 100
    NOCYCLE         -- 번호 순환이 없다. (생략 가능)     CYCLE : 번호가 MAVALUE에 도달하면 다음 번호는 MINVALUE이다.
    NOCACHE         -- 메모리 캐시를 사용하지 않는다.    CACHE : 메모르 캐시를 사용한다.(사용하지 않는 것이 좋다.)
    ORDER           -- 번호 건너뛰기 없다.               NOORDER : 번호 건너뛰기가 가능하다.
;
-- 시퀀스에서 번호 뽑는 함수 : NEXTVAL
-- SELECT DEPARTMENT_SEQ.NEXTVAL FROM DUAL;    -- 테이블에 없는 데이터를 조회하려면 DUAL 테이블을 사용한다.
        
-- 데이터 입력하기(PARENT KEY 먼저 입력해야함)
...
INSERT INTO DEPARTMENT_TBL(DEPT_NO, DEPT_NAME, LOCATION) VALUES(DEPARTMENT_SEQ.NEXTVAL, '총무부', '대구');    -- 이와 같이 특정 칼럼 빼고 데이터를 넣고 싶을때 사용한다
...
-- 데이터 수정하기
-- 1. 부서번호가(DEPT_NO)가 1인 부서의 지역(LOCATION)을 '경기'로 수정하시오
UPDATE DEPARTMENT_TBL 
   SET LOCATION = '경기'  -- 수정할 내용(여기서 등호는 대입 연산자)
 WHERE DEPT_NO = 1;       -- 조건문(여기서 등호는 비교 연산자이다.) 
COMMIT;

-- 2. 부서번호(DEPART)가 1인 부서에 근무하는 급여를 500000원 증가시키시오
UPDATE EMPLOYEE_TBL
...
-- 데이터 삭제하기
-- 1. 지역(LOCATION)이 '대구'인 부서를 삭제하시오  (대구에서 근무하는 사원이 없어서 문제없이 삭제된다)
DELETE FROM DEPARTMENT_TBL WHERE LOCATION = '대구';
COMMIT;

-- 2. 지역(LOCATION) 서울인 부서를 삭제
DELETE FROM DEPARTMENT_TBL WHERE LOCATION = '서울';   -- 위 에서 DEPART는 ON DELETE SET NULL 외래키 옵션에 의해서 DEPART만 NULL로 바뀌고 나머진는 그대로
COMMIT;  
