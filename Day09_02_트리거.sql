...
--TRID1 동작 확인 (EMPLOYEES 테이블의 데이터를 수정해 본다.)
UPDATE EMPLOYEES
    SET LAST_NAME = 'Queen'
...
UPDATE EMPLOTEES SET FIRST_NAME = 'kim' WHERE EMPLOYEE_ID = 99;
...
/*
    트리거 실습
    사원 테이블에서 삭제할 사원정보를 퇴사가 테이블로 삽입하는 RETIRE_TRIG 생성
    
    작업 순서
    1. 사원 테이블(EMPLOYEES)의 구조를 복사하여 퇴사자(RETIRES) 테이블을 생성한다
    2. 퇴사자 테이블에 퇴사번호(RETIRE_ID), 퇴사일자(RETIRE_DATE) 칼럼을 추가한다.
    3. 퇴사번호(RETIRE_ID) 칼럼을 기본키로 지정한다.
    4. 퇴사번호를 생성할 RETIRE_SEQ 시퀀스를 생성한다.
    5. RETORE_TRIG 트리거를 생성한다.
        1) AFTER DELETE 트리거
...
CREATE TABLE RETIRES
    AS(SELECT *
        FROM EMPLOYEES WHERE 1=2);  -- 구조만 변경
