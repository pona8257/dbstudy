...
/*
    ...
    3. 모든 세부 작업이 성공하면 COMMIT하고, 하나라도 실패하면 모든 세부작업의 취소를 진행한다.
        (All or Nothing)
*/
...
-- 4) 테이블 별명 지정하기
SELECT E.ENAME
  FROM EMP E; -- EMP 테이브르이 병명을 E로 부여한다. AS(ALIAS)를 사용할 수 없다.

-- 5) 칼럼 별명 지정하기
SELECT E.ENAME AS 사원명  -- E.ENAME 칼럼의 별명을 '사원명'으로 부여한다. AS(ALIAS)를 사용할 수 있다. 해도되고 안해도됨
  FROM EMP E;


-- 2. 사원 테이블의 모든 칼럼 조회하기
-- 1) * 활용하기(SELECT절에서 *는 모든 칼럼을 의미)
...
--    1) WILD CARD 
--       (1) % : 모든 문자, 글자 수 제한 없는 모든 문자   ATT, AW, AWFDS 등 제한없이 가능
--       (2) _ : 1글자로 제한된 모든 문자                 AS, AF, AG, AR 등 A포함 두글자 
--    2) 연산자
--       (1) LIKE     : WILD CARD를 초함
--       (2) NOT LIKE : WILD CARD를 포함 하지 않는다.
